...
def start(message):
    ...
    g = guarantor[u]
    if g["banned"]:
        bot.send_message(message.chat.id, t["banned"])
        return
    if g["status"] != "verified":
        bot.send_message(message.chat.id, t["not_verified"], reply_markup=InlineKeyboardMarkup().add(
            InlineKeyboardButton("📝 申请入驻", callback_data="apply")
        ))
        return
    bot.send_message(message.chat.id, t["home"], reply_markup=main_menu(u))
    ...
    try:
        if data == "home":
            ...
            if code not in orders or orders[code]["status"] != "open":
                bot.answer_callback_query(call.id, "❌ 订单已被抢走")
                return
            o = orders[code]
            o["status"] = "progress"
            o["guar"] = u
            ...
            com = amt * 0.05
            o["commission"] = com
            bot.answer_callback_query(call.id, "✅ 抢单成功！")
...
def admin_command(message):
    ...
    try:
        if t[0] == "/verify":
            ...
            bot.send_message(message.chat.id, f"✅ 已通过 {uid}")

        elif t[0] == "/ban":
            uid = int(t[1])
            guarantor[uid]["banned"] = True
            bot.send_message(uid, "❌ 您的账号已被冻结")
            bot.send_message(message.chat.id, f"✅ 已封锁 {uid}")

        elif t[0] == "/unban":
            uid = int(t[1])
            guarantor[uid]["banned"] = False
            bot.send_message(uid, "✅ 您的账号已解封")
            bot.send_message(message.chat.id, f"✅ 已解封 {uid}")
            ...
            bot.send_message(message.chat.id, f"✅ 已给 {uid} 加 {amt} USDT")
            ...
            bot.send_message(message.chat.id, f"✅ 已给 {uid} 扣 {amt} USDT")
            ...
            bot.send_message(message.chat.id, f"✅ 已派单 {code} 给 {gid}")

        elif t[0] == "/grab":
            code = t[1]
            amt = float(t[2])
            orders[code] = {"type":"grab","amount":amt,"status":"open"}
            bot.send_message(message.chat.id, f"✅ 已创建抢单 {code} {amt} USDT")

    except Exception as e:
        bot.send_message(message.chat.id, f"❌ 指令错误: {e}")

# 启动（和你用户端完全一样的写法，Railway直接跑通）
