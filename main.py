...
# ======================== 两个机器人 Token ========================
BOT1_TOKEN = "8716451687:AAGXoF5wuwuroCJ23w5UzaueXCUyy5p67q0"   # 机器人A（抢单/审核/派单）
BOT2_TOKEN = "8279854167:AAHLrvg-i6e0M_WeG8coIljYlGg_RF8_oRM"   # 机器人B（口令担保/商家入驻）
...
def user_input_a(msg):
    ...
    lang = user_lang1.get(u,"zh")
    t = TEXT_A[lang]
    user_applying1.setdefault(u,False)
    if not user_applying1[u]:
        return
...
def admin_cmd_a(msg):
    ...
    try:
        ...
        if len(arr)>=2 and arr[0] == "查ID":
            ...
            os = [o for o in orders1.items() if o[1]["user"]==target]
            text = f"📋 用戶 {target}\n💰 餘額：{bal:.2f}\n"
            for oid,o in os:
    ...
    if u == ADMIN_ID_B and txt.startswith("/add"):
        ...
        if len(p) == 3:
            try:
                ...
                bot2.send_message(cid, "❌ 格式 /add ID 金額")
        return

    if u not in user_step2:
        user_step2[u] = None

    if user_step2[u] == "create_amount":
        try:
            amt = float(txt)
            user_step2[u] = {"step": "create_tip", "amount": amt}
            bot2.send_message(cid, t["input_tip"], reply_markup=back_menu2(u))
        except:
            bot2.send_message(cid, "❌ 請輸入數字", reply_markup=back_menu2(u))

    elif isinstance(user_step2[u], dict) and user_step2[u]["step"] == "create_tip":
        amt = user_step2[u]["amount"]
        code = txt
        if user_balance2[u] < amt:
...
def run_botA():
    bot1.infinity_polling(timeout=60)

def run_botB():
    bot2.infinity_polling(timeout=60)

if __name__ == "__main__":
    threading.Thread(target=run_flask, daemon=True).start()
    threading.Thread(target=refresh_virtual_orders1, daemon=True).start()
    threading.Thread(target=run_botA, daemon=True).start()
    threading.Thread(target=run_botB, daemon=True).start()
    print("✅ 双机器人启动成功 —— 机器人A + 机器人B 100% 功能全部保留")
