...
def mining_cooldown(uid, coin):
    ...
    if res:
        try:
            ...
            pass
    exp_time = (datetime.now() + timedelta(minutes=MINING_COOLDOWN)).strftime("%Y-%m-%d %H:%M:%S")
...
def huihu(message):
    ...
    u = get_user(uid)
    if u[3] < 2:
        bot.send_message(message.chat.id, "❌ TRX不足")
        return
    new_trx = u[3] - 2
...
def open_mining(message):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        bot.send_message(message.chat.id, f"✅ 已开通挖矿权限 → {ident}")
    except:
        bot.send_message(message.chat.id, "用法：/open_mining 矿工ID/用户ID")
